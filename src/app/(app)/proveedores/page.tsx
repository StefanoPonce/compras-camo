  ...
  return (
    <>
      ...
      <form className="mb-4" action="/proveedores">
        <input className="campo-input max-w-xs" type="search" name="q" placeholder="Buscar por nombre, contacto o RTN" defaultValue={q} />
      </form>

      {esAdmin && <FormularioCrearProveedor />}

      {proveedores.length ? (
        <div className="tarjeta overflow-x-auto">
          <table className="w-full tabla" style={{ minWidth: 640 }}>
            <thead><tr><th>Proveedor</th><th>Contacto</th><th>Teléfono</th><th>RTN</th><th>Estado</th>{esAdmin && <th></th>}</tr></thead>
            <tbody>
              {proveedores.map((p) => (
                <tr key={p.id}>
